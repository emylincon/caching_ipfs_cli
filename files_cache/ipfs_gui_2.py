...
def plot_performance():
    ...
    plt.bar(ypos, values, align='center', color='m')
...
def plot_changing_freq():
    ...
    for key in plot_dic.keys():
        plt.plot(plot_dic[key], linewidth=5, label=key, color=colour[key])

    plt.title('Changing frequency Graph')
    plt.ylabel('URL')
...
def plot_local_cache_freq():
    ...
    if local_cache_frequency() == 'no items':
        ...
        for key in plot_dic.keys():
            ...
            plt.title('Local Cache frequency Graph')
            plt.ylabel('URL')
            plt.xlabel('Time (seconds)')
            plt.legend()
            plt.pause(2)
...
def plot_resource_util(x, y):
    ...
    plt.ion()
    plt.grid(True, color='k')
    plt.plot(x_axis, linewidth=5, label='RTT')
    plt.plot(y_axis, linewidth=5, label='CPU')
    plt.title('CPU and RTT Utilization over Time')
    plt.ylabel('CPU and RTT')
    plt.xlabel('Time (seconds)')
    plt.legend()
    plt.pause(2)
...
def plot_relative_frequency():
    ...
    plt.ion()
    plt.pie(val, labels=keys, autopct='%.3f%%', shadow=True, explode=explode, colors=cols)
